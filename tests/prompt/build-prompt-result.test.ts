...
test("builds a structured prompt result for a standard request", async () => {
  ...
  const routing = routeRequest(effectiveRequest, scoring, createResolvedConfig());
  ...
  assert.match(promptResult.main_prompt, /FFmpeg slide plan/);
  assert.match(promptResult.main_prompt, /slide_1=/);
...
test("uses novel override theme and duration in prompt result", async () => {
  ...
  const promptResult = buildPromptResult({
    ...
    motionPlan,
    platformOutputSpec,
    resolvedConfig: createResolvedConfig({ video_engine: "veo3", language: "ko" }),
    routing,
    scoring,
    novelShortsPlan,
  });
  ...
  assert.match(promptResult.style_descriptor, /ffmpeg_slides format/);
  assert.match(promptResult.main_prompt, /Novel highlight:/);
  assert.match(promptResult.main_prompt, /duration_sec=25/);
});
